function getDataByWindowUrlKey(){
    ...
    .then(function (response) {
      ...
      $('#product-company').html(response.data.author);
      $('#product-produced').html('Production Starts '+response.data.publication_year+' by '+response.data.publisher);

      // START -- note, unless you have these data in a database structure, here we are hardcoding them for display purposes
        // review
        let stars = randomIntFromInterval(4,5);
        template = '';
        ...
        // tag
        let collectionOfTag = ['High Quality','Guitar','Bright Note','Keyboard','Best Price','Recommended','Imported','USA','Japanese','UK'];
        let selectedTags    = collectionOfTag.sort(() => .4 - Math.random()).slice(0, 4); // only get 4, randomly, from collectionOfTag
        template = '';
